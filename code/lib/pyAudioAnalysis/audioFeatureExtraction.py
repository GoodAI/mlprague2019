...
from code.lib.pyAudioAnalysis import utilities
...
def listWavFeatureExtraction(list_of_wavs, fs, mt_win, mt_step, st_win, st_step,
                            compute_beat=False):
    """
    This function extracts the mid-term features of the loaded WAVE files

    The resulting feature vector is extracted by long-term averaging the mid-term features.
    Therefore ONE FEATURE VECTOR is extracted for each WAV file.

    ARGUMENTS:
        - list_wavs:          the loaded wave files
        - mt_win, mt_step:    mid-term window and step (in seconds)
        - st_win, st_step:    short-term window and step (in seconds)
    """
    ...
    mt_feature_names = []
    for i, x in enumerate(list_of_wavs):        
        print("Analyzing file {0:d} of "
              "{1:d}".format(i+1,
                             len(list_of_wavs)))

        t1 = time.clock()        
        # x = audioBasicIO.stereo2mono(x)  # expected to be done already
        if x.shape[0]<float(fs)/5:
            print("  (AUDIO FILE TOO SMALL - SKIPPING)")
            continue
...
def listsWavFeatureExtraction(list_of_lists_of_wavs, sampling_rate, mt_win, mt_step, st_win, st_step, compute_beat=False):
    '''
    similar to dirsWavFeatureExtraction, but the wavs are already read - only extract features
    '''

    # feature extraction for each class:
    features = []
    for i, li in enumerate(list_of_lists_of_wavs):
        [f, feature_names] = listWavFeatureExtraction(li, sampling_rate, mt_win, mt_step,
                                                      st_win, st_step,
                                                      compute_beat=compute_beat)
        if f.shape[0] > 0:
            # if at least one audio file has been found in the provided folder:
